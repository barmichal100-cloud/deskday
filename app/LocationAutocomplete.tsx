...
type LocationAutocompleteProps = {
  ...
  label?: string;
  error?: string;
  variant?: "home" | "form"; // "home" for the fancy homepage style, "form" for regular forms
  name?: string;
  required?: boolean;
};
...
export default function LocationAutocomplete({
    ...
    try {
      ...
      if (mapboxKey) {
        ...
        for (const f of raw) {
          ...
          const city = f.text || "";
          const country =
            (f.context ?? []).find((c: MapboxContextItem) =>
              String(c.id).startsWith("country.")
            )?.text || "";
          const label =
            placeTypes.includes("country") && !city
              ? country
        ...
        filtered.sort(
          (a: ScoredFeature, b: ScoredFeature) =>
            (b.score || 0) - (a.score || 0) ||
            String(a.place_name).localeCompare(String(b.place_name))
        );
        const providerResults = filtered.map(
          (f) =>
            ({
              id: f.id,
              text: f.text,
      ...
      const out = rawNom
        .map((f, idx) => {
          ...
          if (tokenPrefixMatch && qtokens.length > 1) score += 140;
          if (qtokens.length > 0 && qtokens.every((t: string) => labelNorm.includes(t)))
            score += 60;
          score += Math.max(0, 20 - labelNorm.length);
        ...
        .map(
          (f) =>
            ({ id: f.id, text: f.text, place_name: f.place_name, context: f.context } as MapboxFeature)
        ) as MapboxFeature[];
  ...
  function scheduleFetch(query: string) {
    ...
    debounceRef.current = window.setTimeout(async () => {
      try {
        setLoading(true);
        const items = await fetchPlaceSuggestions(query, "place");
        setSuggestions(items as MapboxFeature[]);
      } catch (err) {
  ...
  const handleSelect = (suggestion: MapboxFeature) => {
    const country = suggestion.context?.find((c: MapboxContextItem) =>
      c.id.startsWith("country.")
    )?.text || "";
    onChange(`${suggestion.text}, ${country}`);
    setShowDropdown(false);
  };

  // Home variant (fancy with icons)
  if (variant === "home") {
    return (
      <div className="flex-1 relative px-6 py-3">
        ...
        <input
          ...
          autoComplete="off"
          required={required}
          className={`w-full bg-transparent text-sm text-gray-700 placeholder:text-gray-400 outline-none ${className}`}
        />
  ...
  // Form variant (standard form input)
  return (
    <div className="relative">
      ...
      <input
        ...
        autoComplete="off"
        required={required}
        className={`w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 ${className}`}
      />
      {loading && <div className="text-xs text-gray-600 mt-1">Loading…</div>}
      {showDropdown && suggestions.length > 0 && (
